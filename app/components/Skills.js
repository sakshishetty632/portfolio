...
const skills = [
 ...
 export const Skills = () => {
    ...
    return (
        <div id='skills' className="min-h-screen mt-12 md:mt-0 mx-4 md:mx-0 xl:my-20 2xl:my-0 pt-8 pb-28 bg-gray-100 text-teal-700">
            ...
            <div className="md:w-1/2 overflow-x-auto scroll-hide lg:w-1/3  mx-auto mt-6 bg-white dark:bg-grey-800 p-2 flex justify-between items-center gap-3 rounded-md">
                {categories.map((c, i) => (
                    <span key={i} onClick={() => setCategory(c)} className={`p-1.5 md:p-2 text-sm md:text-base w-full text-center cursor-pointer rounded-md ${category.toLowerCase() === c.toLowerCase() ? "bg-teal-900 text-white" : "bg-white dark:bg-grey-800 hover:bg-gray-100 hover:dark:bg-grey-900"} transition-all capitalize`}>{c}</span>
                ))}
            </div>
            ...
            {skills.filter((s) => s.category.toLowerCase() === category.toLowerCase()).map((skill, index) => (
                    <div key={index} className="flex flex-col justify-center items-center gap-2">
                        <div title={skill.name}
                            className="h-20 w-20 md:h-24 md:w-24 rounded-full bg-white flex items-center justify-center shadow-md">
                            <Image
                                alt={skill.name}
                                width={100}
                                height={100}
